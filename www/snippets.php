				...
				<div class="mainformDiv">
					<form id="snippetAddForm" name="snippetAddForm" method="post" action="lib/crud/snippets.crud.php" enctype="multipart/form-data" class="myform stylizedForm stylized" style="width:100%;">
						...
						<div>
							...
							<input name="snippetName" id="snippetName" size="75" tabindex='1' <?php if(isset($errors['snippetNameVal'])){ echo 'value="'.$errors['snippetNameVal'].'"';}?>>
							<div  id="errorDiv" style="float:left;margin-left:220px; margin-top:-10px; margin-bottom:10px;">
							<?php if(isset($errors['snippetName'])){echo "<span class=\"error\">".$errors['snippetName']."</span>";}?>
							</div>
						</div>
						<div style="float:left;">
							<label for="snippetDesc"><font color="red">*</font> Snippet Description:</label>
							<input name="snippetDesc" id="snippetDesc" size="150" tabindex='2'  <?php if(isset($errors['snippetDescVal'])){ echo 'value="'.$errors['snippetDescVal'].'"';}?>>
							<div id="errorDiv" style="float:left;margin-left:220px; margin-top:-10px; margin-bottom:10px;">
							<?php if(isset($errors['snippetDesc'])){echo "<span class=\"error\">".$errors['snippetDesc']."</span>";}?>
							</div>
						</div>

						<div style="clear:both">
							<label for="snippet"><font color="red">*</font> Snippet:</label>
							<textarea rows="8" cols="100" wrap="off" id="snippet" name="snippet" tabindex='6' style="width:500px;margin-left:10px;float:clear;" <?php if(isset($errors['snippetVal'])){ echo 'value="'.$errors['snippetVal'].'"';}?>/>
							</textarea> 
							<div class="spacer"></div>
							<div  id="errorDiv" style="float:left;margin-left:77px; margin-top:-10px; margin-bottom:10px;">
								<?php if(isset($errors['snippet'])){echo "<span class=\"error\">".$errors['snippet']."</span>";}?>
							</div>						
						</div>
