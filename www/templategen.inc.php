...
$rs              = $db->q('SELECT COUNT(*) AS total FROM configtemplates');
	...
	<legend>Search</legend>
		<form name ="searchForm" method="GET" action="templategen.php" onsubmit="return searchValidateForm()">
			<select name="searchColumn" id="searchColumn" class="paginate">
				<option value="template">Config Template</option>
...
$q     = $db->q("SELECT id, templateName, templateDesc, template
...
<table id="templateTbl" class="tableSimple">
	<thead>
		<th><input type="checkbox" disabled="disabled"/></th>
		<th align="left">Name</th>
		<th align="left">Description</th>
		<th align="left">Config Template</th>
	</thead>
	<tbody>
	<?php
	foreach ($result['rows'] as $rows):
		...
		$templateDesc = $rows['templateDesc'];
		...
		echo '<td align="left">' . $templateDesc . '</td>';
