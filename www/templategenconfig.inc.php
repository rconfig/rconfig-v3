...
$rs              = $db->q('SELECT COUNT(*) AS total FROM generatedConfigs');
...
<div id="deviceActionDiv">
	...
	<legend>Search</legend>
		<form name ="searchForm" method="GET" action="templategen.php" onsubmit="return searchValidateForm()">
...
$q     = $db->q("SELECT id, configName, templateName, configFilename, configDate
...
<table id="configTbl" class="tableSimple">
	<thead>
	...
	foreach ($result['rows'] as $rows):
		...
		$configFilename = $rows['configFilename'];
		$configDate = $rows['configDate'];
		
		// limit template output to table to 50 chars
		$template = (strlen($template) > 51) ? substr($template,0,50).'...' : $template; 
		...
		echo '<td align="left">' . $configName . '</td>';
		echo '<td align="left">' . $templateName . '</td>';
		echo '<td align="left">' . $configFilename . '</td>';
		echo '<td align="left">' . $configDate . '</td>';
		echo '</tr>';
	endforeach;
	?>
